...
from utilities.gig_types import GigDefinition, Gig
from utilities.controller_helper import GIG_DEFINITION_ANNOTATION

OPERATIONS = ['CREATE','UPDATE']

@kopf.on.mutate(CronJob.version,
                CronJob.plural,
                annotations={GIG_DEFINITION_ANNOTATION: kopf.PRESENT},
...
def onmutatecronjob(patch, meta, annotations, logger, **_):
    ...
    logger.info(f'JobTemplate for CronJob {meta.name} annotated => {GIG_DEFINITION_ANNOTATION}: {annotations[GIG_DEFINITION_ANNOTATION]}')

@kopf.on.validate(CronJob.version,
                  CronJob.plural,
                  annotations={GIG_DEFINITION_ANNOTATION: kopf.PRESENT},
                  operations=OPERATIONS) # type: ignore
def onvalidatecronjob(annotations, meta, **_):
    ...
    if (not gigDef.exists()):
        raise kopf.AdmissionError(f'The GigDefinition for the {GIG_DEFINITION_ANNOTATION} annotation in CronJob {meta.name} does not exist.', code=499)
