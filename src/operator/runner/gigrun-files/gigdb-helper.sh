...
function stageEnvExists() {
    ( { set +x; } 2>/dev/null; gigdb-cli --raw HISMEMBER ${STAGE_ID}_ENV "${1}" )
}

function stageEnvGet() {
    ( { set +x; } 2>/dev/null; gigdb-cli --raw HGET ${STAGE_ID}_ENV "${1}" )
}

function stageEnvGetCsv() {
    ( { set +x; } 2>/dev/null; gigdb-cli --raw --csv HGETALL ${STAGE_ID}_ENV )
}

function stageEnvGetJson() {
    ( { set +x; } 2>/dev/null; gigdb-cli --raw HGETALL ${STAGE_ID}_ENV | paste -d "=" - - | jo )
}

function stageEnvKeys() {
    ( { set +x; } 2>/dev/null; gigdb-cli --raw HKEYS ${STAGE_ID}_ENV )
}

function stageEnvSet() {
    ( { set +x; } 2>/dev/null; gigdb-cli --raw HSET ${STAGE_ID}_ENV "${1}" "${2}" > /dev/null )
}

function stageEnvValues() {
    ( { set +x; } 2>/dev/null; gigdb-cli --raw HVALS ${STAGE_ID}_ENV )
}
